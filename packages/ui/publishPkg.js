...
if (args.includes("--prepare")) {
  ...
  //@cobuild/database
  if (packageJson.dependencies["@cobuild/database"] === "workspace:*") {
    const latestVersion = getLatestNpmVersion("@cobuild/database");
    if (latestVersion) {
      packageJson.dependencies["@cobuild/database"] = latestVersion;
      fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2) + "\n");
      console.log("Prepared for publishing.");
    }
  }
...
if (args.includes("--revert")) {
  ...
  //@cobuild/database
  if (packageJson.dependencies["@cobuild/database"] !== "workspace:*") {
    packageJson.dependencies["@cobuild/database"] = "workspace:*";
    fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2) + "\n");
    console.log("Reverted to workspace.");
  }
}
